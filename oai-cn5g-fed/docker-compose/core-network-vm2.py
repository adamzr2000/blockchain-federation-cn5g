...
BASIC_W_NRF = 'docker-compose-basic-nrf-vm2.yaml'
...
COMPOSE_CONF_MAP = {
    'docker-compose-mini-nrf.yaml': 'vm2-conf/mini_nrf_config.yaml',
    'docker-compose-mini-nonrf.yaml' : 'vm2-conf/mini_nonrf_config.yaml',
    'docker-compose-basic-nrf-vm2.yaml' : 'vm2-conf/basic_nrf_config.yaml',
    'docker-compose-basic-vpp-nrf.yaml' : 'vm2-conf/basic_vpp_nrf_config.yaml',
    'docker-compose-basic-nrf-ebpf.yaml' : 'vm2-conf/basic_nrf_config_ebpf.yaml',
    'docker-compose-basic-vpp-pcf-redirection.yaml' : 'vm2-conf/redirection_steering_config.yaml',
    'docker-compose-basic-vpp-pcf-steering.yaml' : 'vm2-conf/redirection_steering_config.yaml'
}
...
class CoreNetwork():
    def __init__(self):
        self.NRF_IP_ADDRESS='192.168.71.130'
        self.AMF_IP_ADDRESS='192.168.71.132'
        self.SMF_IP_ADDRESS='192.168.71.133'
        self.UPF_IP_ADDRESS='192.168.71.134'
        self.AUSF_IP_ADDRESS='192.168.71.138'
        self.UDM_IP_ADDRESS='192.168.71.137'
        self.UDR_IP_ADDRESS='192.168.71.136'
...
def check_config(file_name):
    ...
    if args.scenario == '1':
        ...
        if file_name == BASIC_VPP_W_NRF or file_name == BASIC_VPP_W_NRF_REDIRECT or file_name == BASIC_VPP_W_NRF_STEERING:
            cmd = f'{curl_cmd}"UPF" | grep -o "192.168.71.201"'
        elif file_name == BASIC_EBPF_W_NRF:
            cmd = f'{curl_cmd}"UPF" | grep -o "192.168.71.129"'
        else:
    ...
    # Only the Mini-No-NRF is supported anymore.
        ...
        logging.debug('\033[0;34m Checking if SMF is able to connect with UPF\033[0m....')
        cmd1 = 'docker logs oai-smf 2>&1 | grep "Received N4 ASSOCIATION SETUP RESPONSE from an UPF"'
        cmd2 = 'docker logs oai-smf 2>&1 | grep "Resolve IP Addr 192.168.71.134, FQDN oai-upf"'
        upf_logs1 = run_cmd(cmd1)
        upf_logs2 = run_cmd(cmd2)
        if upf_logs1 is None or upf_logs2 is None:
            logging.error('\033[0;31m UPF did not answer to N4 Association request from SMF\033[0m....')
